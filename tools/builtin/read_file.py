...
class ReadFileParams(BaseModel):
    ...
    #offset defines from where to start reading the file
    offset: int = Field(1, ge=1, description="line number to start reading from (1-based). Defaults to 1") #1 id defautl , ge means greater than or equals to
    limit: int | None = Field(

    )
class ReadFileTool(Tool):
    name = "read_file",
    #gives llm description about when and how to use the tool
