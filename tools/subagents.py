...
GITHUB_ISSUE_CREATOR = SubagentDefinition(
    name="github_issue_creator",
    description="Creates GitHub issues using an MCP GitHub server",
    goal_prompt="""You are a GitHub issue creation specialist.
Your job is to create a GitHub issue using the MCP GitHub tool.

REQUIREMENTS:
- Extract repository owner/name, title, and body from the user's request
- Optional: labels, assignees, milestone
- Use the MCP tool named github__create_issue

If required details are missing, ask for them.
Return the created issue URL when successful.
