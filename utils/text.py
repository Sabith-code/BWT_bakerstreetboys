...
def truncate_text(text: str,
    ...
    if target_tokens <=0:
        return suffix.strip()
    if preserve_lines:
        return _truncate_by_lines(text, model, target_tokens, suffix)
    else:
        return _truncate_by_chars(text, model, target_tokens, suffix)
    
def _truncate_by_lines(text: str, model:str, target_tokens:int, suffix:str) -> str:
    ...
    for line in lines:
        ...
        if not result_lines:
            #even a single line is too long, truncate by chars
            return _truncate_by_chars(text, model, target_tokens, suffix)
